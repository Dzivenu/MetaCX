...
export function InboundDisclaimerSettings() {
  const { settings, updateSettings, isLoading } = useOrgSettings();
  const [disclaimerHtml, setDisclaimerHtml] = useState("");
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (settings?.thermalReceiptInboundCryptoOrderDisclaimerHtml !== undefined) {
      setDisclaimerHtml(settings.thermalReceiptInboundCryptoOrderDisclaimerHtml);
    }
  }, [settings]);
  ...
  const handleSave = async () => {
    ...
    await updateSettings({
      thermalReceiptInboundCryptoOrderDisclaimerHtml: disclaimerHtml,
    });
    setSaving(false);
  };
  ...
  return (
    <Stack gap="md">
      <Title order={4}>Inbound Crypto Receipt Disclaimer</Title>
      <Text size="sm" c="dimmed">
        Disclaimer text for inbound crypto transactions
      </Text>
      <Text size="sm" c="dimmed">
        Placeholders:
        <List size="sm">
          <List.Item>__WALLET__ - displays the inbound (our) crypto address</List.Item>
